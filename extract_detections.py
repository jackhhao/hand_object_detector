...
def parse_args():
	...
	parser.add_argument('--buffer_size', default=5,
						type=int)
...
if __name__ == '__main__':
	...
	with torch.no_grad():
		...
		# Set up webcam or get video path
		if webcam_num >= 0 :
			cap = cv2.VideoCapture(webcam_num)
			if not cap.isOpened():
				raise RuntimeError("Webcam could not open. Please check connection.")
		else:
			print(f'vid path = {args.vid_path}')
			print(f'save dir = {args.save_dir}')
			cap = cv2.VideoCapture(args.vid_path)
			if not cap.isOpened():
				raise RuntimeError("Could not open video.")

		# timeseries of T/F for whether contact is detected
		detection_buffers = [
	  		deque(buffer_size*[False], buffer_size), # left hand
			deque(buffer_size*[False], buffer_size) # right hand
		]
  
		# current cumulative contact state for each hand (after accounting for buffer)
		curr_detected = [False, False]
  
		# queue of last buffer_size video frames
		...
		while True:
			...
			for j in range(1, len(pascal_classes)):
				...
				if inds.numel() > 0:
					...
					else: # if we want to visualize all classes (inc. background), not just hand + object
						if all_dets is None:
			...
			if vis:
				...
				# im2show = vis_detections_filtered_objects_PIL(np.array(im2show), all_dets, hand_dets, thresh_hand, thresh_obj)

			misc_toc = time.time()
			nms_time = misc_toc - misc_tic

			# if obj_dets is not None and hand_dets is None:
			# 	print("objects detected w/o hands")
			...
			if hand_dets is not None:
				for hand_idx, i in enumerate(range(np.minimum(10, hand_dets.shape[0]))):
					...
					# print(lr)
					state = hand_dets[i, 5]
					new_vals[int(lr)] = score > thresh_hand and state == 3
			...
			for i,buffer in enumerate(detection_buffers):
				...
				if old_frame is not None:
					...
					result_path = os.path.join(folder_name, f'frame_{int(time.time())}_{condition}.png')
			...
			# 		save_frame |= (score > thresh_hand and state == 3)
				
				# if save_frame:
				# 	folder_name = args.save_dir
				# 	os.makedirs(folder_name, exist_ok=True)
				# 	result_path = os.path.join(folder_name, f'frame_{int(time.time())}.png')
